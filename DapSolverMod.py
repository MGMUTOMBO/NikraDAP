...
Debug = False
# =============================================================================
def makeDapSolver(name="DapSolver"): #TODO: USE THIS TO CREATE THE OPTIMIZER
    ...
    DapSolverC(objSolver) #initialiezed the propertie of the object
    ...
    return objSolver
# =============================================================================
class CommandDapSolverC: #TODO: USE THIS FUNCTION TO CREATE THE OPTIMIZER COMMAND
    ...
    #  -------------------------------------------------------------------------
    def Activated(self): #TODO: USE THIS WAY TO ACTIVATE THE OPTIMIZER
        """Called when the Solver command is run"""
        #the function prevent to have more that one solver in a container
        if Debug:
            DT.Mess("CommandDapSolverC-Activated")

        # Re-use the old solver object if it exists
        # Otherwise create a new solver object
        ##this maybe used to to integrate the optimizer the same way and run it
        activeContainer = DT.getActiveContainerObject() #give the list of active container in FreeCadGui
        solverObject = None
        for groupMember in activeContainer.Group: # give the list of component in the active container
            if "DapSolver" in groupMember.Name: # give the name of each composant of the active cintainer
